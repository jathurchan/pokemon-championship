    ...
    void ResourceHolder::loadStateAssets(sf::View &view) {
        ...
        for (const Json::Value &stateAssets: assetsInfo) {
            if (stateAssets != Json::nullValue) {
                ...
                stateAssetsMap[index].first.reserve(stateAssets["text"].size() + stateAssets["button"].size());
                stateAssetsMap[index].second.reserve(stateAssets["sprite"].size() + stateAssets["button"].size());
